...
export class RealtimeClient extends RealtimeEventHandler {
    ...
    private _realtimeConversation: RealtimeConversation;
    ...
    constructor(openWebsocketConnection: () => Promise<WebSocket> | WebSocket, debug: boolean = false) {
        ...
        this._debug = debug;
        this._realtimeConversation = new RealtimeConversation();
    }
    ...
    */
    ...
    disconnect() {
        ...
        this._realtimeConversation.clear();
    }
    ...
    _addAPIEventHandlers() {
        ...
        const handler = (event: any, ...args: any) => {
            const { item, delta } = this._realtimeConversation.processEvent(event, ...args);
            return { item, delta };
        };
    ...
    createResponse(): boolean {
        if (
            ...
            this._realtimeConversation.queueInputAudio(this._inputAudioBuffer);
            this._inputAudioBuffer = new Int16Array(0);
        }
    ...
    cancelResponse(id: string, sampleCount = 0): { item: AssistantItemType | null } {
        if (!id) {
            ...
            const item = this._realtimeConversation.getItem(id);
            if (!item) {
            ...
            this._realtimeApi.send('conversation.item.truncate', {
                ...
                audio_end_ms: Math.floor(
                    (sampleCount / this._realtimeConversation.defaultFrequency) * 1000,
                ),
            });
            return { item };
        }
